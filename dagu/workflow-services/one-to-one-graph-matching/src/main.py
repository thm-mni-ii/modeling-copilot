#!/usr/bin/env python3
"""Stub service boundary for future one-to-one graph matching."""
import json
import os
import sys
import time

SERVICE = "one-to-one-graph-matching"
...
def main() -> int:
    ...
    print(f"[{SERVICE}] dummy matching started", file=sys.stderr); time.sleep(delay)
    result = {"score": 0.8, "mapping": [{"submissionNode": "n1", "referenceNode": "n1", "confidence": 0.8}]}
    print(json.dumps({"status": "ok", "capability": SERVICE, "step": payload.get("step", SERVICE), "summary": "Dummy-1:1-Matching abgeschlossen", "result": result}))
    return 0
